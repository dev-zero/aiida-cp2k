...
def test_section_prefixes():
    # section prefixes are only allowed for actual sections, not bare keywords
    inp = Cp2kInput({"&FOO": "bar"})
    with pytest.raises(ValueError):
        inp.to_string()

    # make sure we don't get a double section prefix
    inp = Cp2kInput({"&FOO": {}})
    assert (
        inp.to_string()
        == """\
...
&END FOO"""
    ...
    # using the same name for a key and a section is not only
    # allowed in CP2K input file syntax but also used in some cases
    inp = Cp2kInput({"&FOO": {}, "FOO": "bar"})
    assert (
        inp.to_string()
        == """\
!!! Generated by AiiDA !!!
